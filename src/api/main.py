...
Two modes, matching LD7236 section 3.4:
- /experiment/{policy_id}: run against an APP-350 policy already indexed,
  and also return the ground-truth comparison.
- /audit: accept arbitrary uploaded privacy-policy HTML, run the same
  pipeline, no ground-truth comparison (demonstration mode).
...
# Allow the local frontend (opened as a static HTML file, or served on
# any localhost port) to call this API from the browser. Restrict
# allow_origins if you deploy this beyond your own machine.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
async def audit_uploaded_policy(file: UploadFile, top_k: int = 10):
    """Demonstration mode: no ground truth, no persistence to the shared
    vector DB — builds an ephemeral in-memory index for this one document."""
    ...
    # ephemeral collection so uploaded documents don't pollute experiment data
    ...
    client.delete_collection("ad_hoc_uploads")  # don't accumulate across uploads
