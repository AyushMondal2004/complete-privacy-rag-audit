"""
RQ4 qualitative review sample generator.

Runs the full pipeline (retrieval + LLM extraction) on a random sample
of policies and saves the RAW predicted JSON (practice, modality,
evidence, explanation) — not just aggregate metrics — so a human can
manually review whether each explanation is:
    - grounded: does the evidence text actually support the explanation?
    - understandable: plain language, no unexplained jargon?
    - not misleading: does it avoid overstating what the evidence shows?

This does NOT do the qualitative judgement itself — that's a human
task per your project's Data Analysis Methods #4. This script only
produces the sample and a blank review sheet to fill in.
