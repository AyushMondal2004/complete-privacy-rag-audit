"""
Results visualisation — generates publication-quality charts for the
dissertation Results chapter.

Produces:
    results/figures/config_comparison_bar.png
    results/figures/per_policy_f1_boxplot.png
    results/figures/category_heatmap.png
    results/figures/threshold_sensitivity.png
    results/figures/error_breakdown.png
...
def chart_category_heatmap():
    ...
    df = pd.read_csv(path)
    # Pivot: rows = categories, columns = configs, values = f1
...
def chart_threshold_sensitivity():
    ...
    for config, colour in CONFIG_COLOURS.items():
        ...
        # Also plot precision and recall as dashed
